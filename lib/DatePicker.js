...
class DatePicker extends Component<void, Props, State> {
    ...
    renderHeader(value){
        const { headerFormat, customHeader } = this.props;
        if(customHeader){
     ...
     */
    render() {
        const { min, max, theme, dateConfig, confirmText, cancelText, showHeader, showFooter, showCaption } = this.props;
        const value = this.state.value;
